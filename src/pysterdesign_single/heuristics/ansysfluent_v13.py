...
class ansysfluent_v13(object):
    ...
    def __validation(self, params: Dict) -> bool:
        ...
        if("benchmark" not in params):
            ...
            if(params["benchmark"] not in self.KNOWN_BENCHMARK_TYPES):
                print("Invalid benchmark, must be one of the following: "+ ", ".join(self.KNOWN_BENCHMARK_TYPES))
                return False
        ...
        if("networkTech" not in params):
            ...
            if(params["networkTech"] not in self.KNOWN_NETWORK_TECH):
        ...
        if("AllowThroughput" not in params):
            params["AllowThroughput"] = self.ALLOW_THROUGHPUT_DEFAULT
        else:
    ...
    def execute(self,parameters: Dict) -> float:
        if self.__validation(parameters): # type: ignore
            return self.performance(parameters)
        else:
            print("invalid")
        return None

    def __str__(self):
        return "ansysfluent_v13_Proof"
